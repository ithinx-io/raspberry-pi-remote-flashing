...
# UBoot partition size [in KiB] (will be rounded up to IMAGE_ROOTFS_ALIGNMENT)
UBOOT_PART_SIZE="40960"
# Set alignment to 4MB [in KiB]
IMAGE_ROOTFS_ALIGNMENT="4096"
# Yocto generated partition size
BOOT_PART_SIZE_ALIGNED="71034"

UBOOT_PART_SIZE_ALIGNED=$(( UBOOT_PART_SIZE + IMAGE_ROOTFS_ALIGNMENT - 1 ))
UBOOT_PART_SIZE_ALIGNED=$(( UBOOT_PART_SIZE_ALIGNED - (( UBOOT_PART_SIZE_ALIGNED % IMAGE_ROOTFS_ALIGNMENT)) ))
SDIMG_SIZE=$(( IMAGE_ROOTFS_ALIGNMENT + UBOOT_PART_SIZE_ALIGNED + BOOT_PART_SIZE_ALIGNED + ROOTFS_PT_SIZE))

UBOOT_PARTITION_START="${IMAGE_ROOTFS_ALIGNMENT}"
UBOOT_PARTITION_END=$(( UBOOT_PART_SIZE_ALIGNED + IMAGE_ROOTFS_ALIGNMENT ))
BOOT_PARTITION_END=$(( UBOOT_PARTITION_END + BOOT_PART_SIZE_ALIGNED ))
# Offset wich comes form yocto
ROOTFS_PARTITION_START=$(( BOOT_PARTITION_END + 2694 ))
...
echo "sudo parted -s ${SDIMG} -- unit KiB mkpart primary ${SDIMG_ROOTFS_TYPE} ${ROOTFS_PARTITION_START} -1s"
